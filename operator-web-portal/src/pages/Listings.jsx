...
const StatusBadge = ({ status }) => {
  ...
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold capitalize ${cls}`}>{status}</span>;
};

const CatBadge = ({ cat }) => (
  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-blue-50 text-blue-700 border border-blue-100">{cat}</span>
);
...
export default function Listings() {
  ...
  const handlePause = async (id, currentStatus) => {
    ...
    const res = await hostAPI.editListing(id, { status: newStatus });
    setListings(prev => prev.map(l => l._id === id ? res.data.experience : l));
  };
      ...
      <div id="listings-page" className="max-w-5xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 max-w-lg">Manage your outdoor experiences, modify trek details, and monitor booking status across your catalog.</p>
          </div>
          <button id="btn-create-listing-header" onClick={() => navigate('/listings/new')}
            className="flex items-center gap-2 bg-[#1A5F45] hover:bg-[#145038] text-white px-5 py-3 rounded-xl text-sm font-semibold transition flex-shrink-0">
            <span className="text-lg leading-none">⊕</span> Create New Listing
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5">
              <svg className="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
              <input id="listings-search" type="text" placeholder="Filter by name..."
          ...
          <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-gray-100">
            <svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M4 6h16M7 12h10M10 18h4"/></svg>
            <span className="text-xs text-gray-500 font-medium">More Filters</span>
          </div>
        </div>
            ...
            <>
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {paginated.length === 0 ? (
                    <tr><td colSpan={5} className="text-center text-gray-400 py-12 text-sm">No listings found.</td></tr>
                  ) : paginated.map(exp => (
                    <tr key={exp._id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-4 py-4"><CatBadge cat={exp.category} /></td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button id={`btn-edit-${exp._id}`} onClick={() => navigate(`/listings/${exp._id}/edit`)}
                          ...
                          <button id={`btn-view-${exp._id}`} onClick={() => navigate(`/listings/${exp._id}`)}
                          ...
                          <button id={`btn-delete-${exp._id}`} onClick={() => handleDelete(exp._id)} disabled={deleting === exp._id}
                            className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition" title="Delete">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
                          </button>
                        </div>
                      </td>
                    </tr>
              ...
              <div className="flex items-center justify-between px-5 py-4 border-t border-gray-100">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1}
                    className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 transition text-sm">‹</button>
                  {Array.from({length: Math.min(totalPages,5)},(_,i)=>i+1).map(n => (
                    <button key={n} onClick={() => setPage(n)}
                      className={`w-8 h-8 flex items-center justify-center rounded-lg text-sm font-medium transition ${page===n ? 'bg-[#1A5F45] text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{n}</button>
                  ))}
